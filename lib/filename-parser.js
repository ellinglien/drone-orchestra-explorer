...
function parseMonthFolder(folderName) {
  ...
  // Drop themes that are just the month name (e.g. Drive folder
  // "2026-06 June" → theme "June" is redundant with the name "June 2026"
  // and would render as "June 2026 - June" in the UI). Also drop themes
  // that only differ in whitespace or casing.
  let cleanTheme = theme ? theme.trim() : null;
  if (cleanTheme && cleanTheme.toLowerCase() === monthName.toLowerCase()) {
    cleanTheme = null;
  }
